...
import com.happy.gene.pdfreport.pdf.*;
...
public class PageDef extends AbstractDef implements IXml<PageDef> {
    ...
    private int pageEndNumberInPdf  = 0;
    ...
    public PdfPage generate(Document pdf, PageDef pageDef) throws Exception {
        ...
        for (Object com : components) {
            ...
            if (com instanceof IDrawable) {
                ...
                if (com instanceof AbstractDef && ((AbstractDef) com).isVisible()) {
                    preCom = com;
                }
            }
        }
        page = pdf.getPdfDocument().getLastPage();
        pageEndNumberInPdf = pdf.getPdfDocument().getPageNumber(page);
        pageEndNumber = (pageEndNumberInPdf - getPageStartNumberInPdf()) + pageStartNumber;

        if (null!=pageDefRenderer) {
            pageDefRenderer.afterRenderer(pdf, pageDef);
        }
        return page;
    }
    ...
    public PageDef clone() {
        ...
        newOne.setCatalog(this.getCatalog());
